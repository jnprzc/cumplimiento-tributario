  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        <span className="font-semibold text-slate-800 text-sm">{cat.emoji} {cat.label}</span>
...
export function Dashboard({ result, businessName = 'Tu negocio', onReset, onShare }: DashboardProps) {
  ...
  return (
    <div className="animate-fade-in pb-28 md:pb-10">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 flex items-center gap-4">
            ...
            <div className="flex flex-col items-end gap-2 shrink-0">
              <span className="text-xs text-slate-400 text-right leading-snug">{weekLabel()}</span>
              {onShare && (
                <button onClick={onShare}
                  className="flex items-center gap-1 text-xs text-pulso font-semibold hover:text-pulso-700 transition-colors">
          ...
          <div>
            ...
            <Link href="/aprender">
              <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden active:scale-[0.98] transition-transform">
                <div className="h-24 bg-pulso-200/60 flex items-center justify-center">
                  <div className="w-10 h-10 rounded-full bg-white/90 shadow-md flex items-center justify-center">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#2D7A4F">
      ...
      {/* Footer */}
      <p className="text-xs text-slate-400 text-center leading-relaxed px-2 pt-6">
        Diagnóstico orientativo. No reemplaza asesoría profesional. Ley 1581 de 2012.
      </p>
      <button onClick={onReset}
        className="w-full py-3 text-sm text-slate-400 hover:text-pulso transition-colors mt-1">
